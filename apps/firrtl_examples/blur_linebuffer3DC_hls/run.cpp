...
int main(int argc, char **argv) {
    ...
    BufferMinimal<uint8_t> in(3, 260, 258);
    ...
    BufferMinimal<uint8_t> out_native(3, 256, 256);
    ...
    BufferMinimal<uint8_t> out_hls(3, 256, 256);
    ...
    for (int y = 0; y < out_native.dim(2).extent(); y++) {
        for (int x = 0; x < out_native.dim(1).extent(); x++) {
            ...
            out_native1(x, y) = out_native(1, x, y);
            out_native2(x, y) = out_native(2, x, y);
            out_hls0(x, y) = out_hls(0, x, y);
            out_hls1(x, y) = out_hls(1, x, y);
            out_hls2(x, y) = out_hls(2, x, y);
        }
    }
