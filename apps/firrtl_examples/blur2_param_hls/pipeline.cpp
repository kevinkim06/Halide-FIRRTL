...
public:
    MyPipeline() : input(UInt(8), 2, "input"),
                   ...
                   output("output") {
        in(x, y) = input(x, y);
        blur_x(x, y) = in(x, y) * weight(x%4,y%4)
                     + in(x+1, y) * weight((x+1)%4,y%4);
        blur_y(x, y) = blur_x(x, y) * weight(x%4,y%4)
                     + blur_x(x, y+1) * weight(x%4,(y+1)%4);
        hw_output(x, y) = select(enable==true, blur_y(x, y), in(x, y));
        output(x, y) = hw_output(x, y);

        weight.dim(0).set_bounds(0, 4);
        weight.dim(1).set_bounds(0, 4);
        weight.dim(0).set_stride(1);
        weight.dim(1).set_stride(4);
