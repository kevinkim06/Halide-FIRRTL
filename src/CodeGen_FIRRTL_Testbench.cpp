...
public:
    ...
    vector<FIRRTL_Argument> arguments(const Scope<CodeGen_FIRRTL_Base::Stencil_Type> &scope);
...
vector<FIRRTL_Argument> FIRRTL_Closure::arguments(const Scope<CodeGen_FIRRTL_Base::Stencil_Type> &streams_scope) {
    ...
    for (const pair<string, Type> &i : vars) {
        ...
        if(ends_with(i.first, ".stream") ||
           ends_with(i.first, ".stencil") ) {
            CodeGen_FIRRTL_Base::Stencil_Type stype = streams_scope.get(i.first);
            is_stencil = true;
            if (ends_with(i.first, ".stream")) is_stream = true;
            if (i.first == outputname) is_output = true;
            res.push_back({i.first, is_stencil, is_stream, is_output, Type(), stype});
            ...
            res.push_back({i.first, is_stencil, is_stream, is_output, i.second, CodeGen_FIRRTL_Base::Stencil_Type()});
        }
    }
    return res;
}
...
CodeGen_FIRRTL_Testbench::CodeGen_FIRRTL_Testbench(ostream &tb_stream,
                                             Target target,
                                             OutputKind output_kind)
    : CodeGen_FIRRTL_Base(tb_stream, target, output_kind, ""),
...
void CodeGen_FIRRTL_Testbench::visit(const Call *op) {
    if (op->name == "stream_subimage") {
        ...
        for (size_t i = 4; i < op->args.size(); i++) {
            if ((i-4)%2==1) {
                store_extents.push_back(*as_const_int(op->args[i]));
            }
        }
        const Variable *stream_name_var = op->args[2].as<Variable>();
        CodeGen_FIRRTL_Base::Stencil_Type stype = stencils.get(stream_name_var->name);
...
void CodeGen_FIRRTL_Testbench::visit(const Block *op) {
    ...
    // This is ugly right now, as the FIRRTL simulation model and DMA programming model
